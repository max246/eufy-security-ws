...
export class LoggingEventForwarder {
  ...
  public stop(): void {
    ...
    if (this.driverLogger) this.driverLogger.settings.attachedTransports = [];
  }
  ...
  private getMessage(logObject: ILogObj, schemaVersion: number): LogMessage {
    ...
    if (schemaVersion <= 20) {
      ...
      if (matches !== null && matches !== undefined && matches.length >= 3) {
        ...
        if (helper[0] !== undefined) typeName = helper[0];
        if (helper[1] !== undefined) functionName = helper[1];
      }
      return {
        ...
        argumentsArray: [logObject["0"] as string],
      };
    }
    return {
